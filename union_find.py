...
class UnionFind: # Disjoint Set Union or Union Find by Rank
    def __init__(self, n):
        ...
        self.size = [1] * n  # Track the size of each component
    ...
    def getSetsInfo(self):
        ...
        # and corresponding rank and members (list of nodes) of each set as values.
        set_info = {}
        for i in range(len(self.parentNode)):
            root = self.find(i)
            # Initialize the set entry if not already present
            if root not in set_info:
                set_info[root] = {'size': self.size[root],'rank': self.treeRank[root], 'members': []}
